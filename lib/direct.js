...
var last = require('es5-ext/lib/Array/prototype/last')
  ...
  , parse = JSON.parse, stringify = JSON.stringify, min = Math.min
  , wsRe = /\s/
  , eolRe = /(?:\r\n|[\n\r\u2028\u2029])/
  , preReturn = /[\s;]/
  , requireRe, breakers, Walker;
...
Walker.prototype = {
	...
	parseRequire: function () {
		...
		match = this.code.slice(this.current).match(requireRe);
		if (!match) {
			this.reportArgsError();
			return;
		}
		try {
	...
	reportArgsError: function () {
		var e = new Error("Cannot parse code" +
			" @" + (this.current) + ": " +

...
var parse           = JSON.parse
  ...
  , ctx = vm.createContext(), runInContext = vm.runInContext
...
Walker.prototype = {
	...
	parseRequire: function () {
		var match = this.code.slice(this.current).match(requireRe), e;
		if (match) {
			this.deps.push({
				value: Function("'use strict'; return " + match[1])(),
				raw: match[1],
				at: this.code.indexOf('(', this.current - 1) + 2
			});
